...
def LSTM_net():
    ...
    inputs = Input(( 4, image_rows_low, image_cols, channel_num))
    # print(K.int_shape(inputs))
    split_x = Lambda(tf.split, arguments={'axis': 1, 'num_or_size_splits': 4})(inputs)
    # frame 0
    x0 = split_x[0]
    ...
    y4 = AveragePooling2D((2,2))(x4)
    y4 = Dropout(dropout_rate)(y4, training=True)
    y43 = K.expand_dims(y4,axis=1)

    lstm_input = K.concatenate([y40,y41,y42,y43], axis=1)
    lstm_out = ConvLSTM2D(filters * 16,kernel_size=(3,3),strides=(1, 1),padding='same',activation='relu')(lstm_input)
    y4 = Dropout(dropout_rate)(lstm_out, training=True)

    y4 = up_block(y4, filters*8, strides=(2,2))
    print(K.int_shape(y4))
    x4 = K.reshape(x4,K.int_shape(y4))
    y3 = concatenate([x4, y4], axis=3)
    y3 = conv_block(y3, filters*8)
    y3 = Dropout(dropout_rate)(y3, training=True)
    y3 = up_block(y3, filters*4, strides=(2,2))
