...
if __name__ == '__main__':
    ...
    # im1 = np.array(Image.open('pyflowlib/examples/car1.jpg'))
    # im2 = np.array(Image.open('pyflowlib/examples/car2.jpg'))
    gt = np.load(os.path.join(gt64,gt64_files[i]))
    gt = point_cloud_to_panorama(gt,
                                v_res = 0.1,# unit meter
                                h_res = 0.5,# unit degree
    ...
    gt1 = np.load(os.path.join(gt64,gt64_files[i+1]))
    gt1 = point_cloud_to_panorama(gt1,
                                v_res = 0.1,# unit meter
                                h_res = 0.5,# unit degree
                                v_height = (-3, 3.4)#unit meters
                                )
    ...
    flow_img = get_opticalflow(gt,gt1)
    print(flow_img.shape)
    # get_opticalflow(im1,im2)
    plt.imshow(flow_img)
    plt.show()
    # print(flow.shape)
    # pred_image = pred[:,:,0]
