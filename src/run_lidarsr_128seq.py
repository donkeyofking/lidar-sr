#!/usr/bin/env python
from models.lidarsr_128_seq_models import *
...
def test_time(iterate_count=50):
    weight_name = "/home/buaaren/lidar-sr/src/train_log/LidarSR/TCN_good_128/weights/weights.h5"
    # load model
...
def test(iterate_count=50):
    ...
    model.load_weights(weight_name)
    test_data = np.load(training_data_file_name)
    total_no = test_data.shape[0]-16

    # load model
    model, _, _ = get_model('testing')
    model.load_weights(weight_name)

    this_test = np.empty([iterate_count, 16, image_rows_low, image_cols, channel_num], dtype=np.float32)
    test_data_prediction = np.empty([total_no, image_rows_high, image_cols, 2], dtype=np.float32)
    ...
    for i in range(total_no):
        ...
        print('Processing {} th of {} images ... '.format(i, total_no))
        ...
        this_prediction_mean = np.mean(this_prediction, axis=0)
        print(this_prediction_mean)
        print(this_prediction_mean.shape)
        this_prediction_var = np.std(this_prediction, axis=0)
        test_data_prediction[i,:,:,0:1] = this_prediction_mean
        test_data_prediction[i,:,:,1:2] = this_prediction_var

    np.save('prediction.npy', test_data_prediction)
